...
& $msbuild (Join-Path $root "tools\AlgorithmProbe\AlgorithmProbe.vcxproj") /m /p:Configuration=$Configuration /p:Platform=x64

$sqliteDll = "L:\vcpkg\installed\x64-windows\bin\sqlite3.dll"
$outputDir = Join-Path $root "build\tools\AlgorithmProbe\x64\$Configuration"
if (Test-Path $sqliteDll) {
    Copy-Item -Force $sqliteDll $outputDir
}
